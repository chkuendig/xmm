	...
	MouseAdapter tablePopupListener = new MouseAdapter() {
		public void mousePressed(MouseEvent e) {
			...
			if ("Delete".equals(((JMenuItem) e.getSource()).getActionCommand())) {
				...
				// Commented out as functionality is not yet done.
				...
				for (int i = 0; i < columnCount; i++) {
					...
					//System.err.println("tablestring len:" + src.getTableString());
					Object val = newColumn.getHeaderValue();
					
					if (val != null && val instanceof FieldModel) {
						System.err.println("FieldModel found!!!!");
						
					}
					newColumn.setHeaderValue(val);
					
					
					//Vector header =  getColumnGroups(TableColumn c, Vector g)
				...
				public DialogImportTable(Dialog parent, File file, int importType) {
					...
					try {
						...
						System.err.println("DialogTable importType:" + importType);
						...
						if (importType == ModelImportSettings.IMPORT_EXCEL) {
							...
							//CSVParser cvsParser = new CSVParser(new FileReader(file));
