...
* @author J. H. S.
*/
public class XMMSimpleHttpRequest implements HttpRequest {
	
	private static final Logger logger = Logger.getLogger(XMMSimpleHttpRequest.class);
	...
	public XMMSimpleHttpRequest(UserAgentContext context, java.net.Proxy proxy) {
		super();
	...
	public synchronized String getResponseText() {
		...
		try {
			...
			logger.warn("getResponseText(): Charset '" + encoding + "' did not work. Retrying with ISO-8859-1.", uee);
			try {
	...
	public synchronized Document getResponseXML() {
		...
		try {
			...
			logger.warn("Unable to parse response as XML.", err);
			return null;
		}
	}
