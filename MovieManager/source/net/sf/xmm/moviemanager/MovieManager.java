...
import net.sf.xmm.moviemanager.util.ClassPathHacker;
...
// 	_movieManager.loadDatabase();
	...
	if (database != null) {
		...
		getMoviesList().setShowsRootHandles(false);
	...
	/* Setting treeCellRenderer */
	getMoviesList().setCellRenderer(new ExtendedTreeCellRenderer(getMoviesListScrollPane(), config.getMovieListHighlightEntireRow()));
...
// 	toolBar.setMaximumSize(toolBar.getPreferredSize());
	...
	if (config.getUseJTreeIcons()) {
	    ((DefaultTreeCellRenderer) getMoviesList().getCellRenderer()).setLeafIcon(new ImageIcon(_movieManager.getImage("/images/movie.png").getScaledInstance(25,25, Image.SCALE_SMOOTH)));
	    ((DefaultTreeCellRenderer)getMoviesList().getCellRenderer()).setClosedIcon(new ImageIcon(_movieManager.getImage("/images/serie.png").getScaledInstance(25,25, Image.SCALE_SMOOTH)));
	    
	    ((DefaultTreeCellRenderer) getMoviesList().getCellRenderer()).setOpenIcon(new ImageIcon(_movieManager.getImage("/images/serie.png").getScaledInstance(25,25, Image.SCALE_SMOOTH)));
	}
	else {
	    ((DefaultTreeCellRenderer) getMoviesList().getCellRenderer()).setLeafIcon(null);
	    ((DefaultTreeCellRenderer) getMoviesList().getCellRenderer()).setClosedIcon(null);
	    ((DefaultTreeCellRenderer) getMoviesList().getCellRenderer()).setOpenIcon(null);
	}
	
	//((javax.swing.plaf.basic.BasicTreeUI)getMoviesList().getUI()).setExpandedIcon(null);
	//((javax.swing.plaf.basic.BasicTreeUI)getMoviesList().getUI()).setCollapsedIcon(null);
	
	((ExtendedTreeCellRenderer) getMoviesList().getCellRenderer()).setRowHeight(config.getMovieListRowHeight());
    }
